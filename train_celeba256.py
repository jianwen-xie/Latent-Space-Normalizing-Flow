...
from model import _netG, _netG256, _netF, weights_init_xavier
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='celeba_hq256', choices=['svhn', 'celeba', 'celeba_crop', 'celeba_hq256'])
    ...
    parser.add_argument('--f_width', default=128, type=int, help='')
    ...
    parser.add_argument('--n_printout', type=int, default=30, help='printout each n iterations')
...
def train(args, output_dir, path_check_point):
    ...
    def plot(p, x):
        return torchvision.utils.save_image(torch.clamp(x, -1., 1.), p, normalize=True, nrow=int(np.sqrt(len(x))))
    ...
    for epoch in range(epoch_start, args.n_epochs):
        ...
        for i, (x, y) in enumerate(dataloader_train, 0):
            ...
            if args.g_is_grad_clamp:
                torch.nn.utils.clip_grad_norm(netG.parameters(), args.g_max_norm)
            optG.step()
            #if jj % 20 == 0:
            ...
            if i % args.n_printout == 0:
                with torch.no_grad():
                    ...
                    z_sample = torch.randn(25, args.nz, 1, 1).to(device)
                    ...
                    plot('{}/samples/recons{:>06d}.png'.format(output_dir, epoch), x_hat[:25])
                    plot('{}/samples/obs{:>06d}.png'.format(output_dir, epoch), x[:25])
                    plot('{}/samples/sample_{:>06d}.png'.format(output_dir, epoch), x_k)
        ...
        if epoch == args.n_epochs or epoch % args.n_metrics == 0:
            try:
                ...
                plot('{}/samples/fid_samples{:>06d}.png'.format(output_dir, epoch), (x_samples[:25] - 0.5) * 2.0)
            ...
            if fid < fid_best and fid < 9999:
                ...
                torch.save(save_dict, '{}/ckpt/best.pth'.format(output_dir))
