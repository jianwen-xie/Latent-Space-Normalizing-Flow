...
from model import _netG, _netF, _netE, weights_init_xavier, _netG_celeba, _netG_mnist, _netG_cifar10
...
def parse_args():
    ...
    parser.add_argument('--prior_type', type=str, default='gaussian', help='type of the prior distribution [flow|ebm|gaussian]')
...
def train(args, output_dir, path_check_point):
    ...
    def sample_langevin_post_z_with_ebm(z, x, netG, netE, verbose=False, mask=None):
        z = z.clone().detach()
        z.requires_grad = True
        ...
        for i in range(args.g_l_steps):
            ...
            z_grad_g_grad_norm = z_grad_g.view(z.shape[0], -1).norm(dim=1).mean()
            z_grad_e_grad_norm = z_grad_e.view(z.shape[0], -1).norm(dim=1).mean()

        if verbose:
            logger.info('Langevin posterior: MSE={:8.3f}, energy={:8.3f}, z_grad_g_grad_norm={:8.3f}, z_grad_e_grad_norm={:8.3f}'.format(g_log_lkhd.item(), en.item(), z_grad_g_grad_norm, z_grad_e_grad_norm))
    ...
    def sample_langevin_post_z_with_gaussian(z, x, netG, verbose=False, mask=None):
        z = z.clone().detach()
        z.requires_grad = True
        ...
        for i in range(args.g_l_steps):
            ...
            z.data = z.data - 0.5 * args.g_l_step_size * args.g_l_step_size * (z_grad_g + 1.0 / (args.e_prior_sig * args.e_prior_sig) * z.data)
        ...
        if verbose:
            logger.info('Langevin posterior: MSE={:8.3f}, z_grad_g_grad_norm={:8.3f}'.format(g_log_lkhd.item(), z_grad_g_grad_norm))
    ...
    if path_check_point:
        ...
        if args.prior_type=='flow':
            ...
            netE.load_state_dict(ckp['netE'])
            optE.load_state_dict(ckp['optE'])
            ...
            for i, x_input in enumerate(dataloader_train, 0):
                ...
                if i % args.n_printout == 0:
                        ...
                        if args.prior_type == 'flow':
                            ...
                            logger.info('{} {:5d}/{:5d} {:5d}/{:5d} '.format(job_id, epoch, args.n_epochs, i,
                            ...
                            num_step = epoch * len(dataloader_train) + i
                            tb_writer.add_scalar("train/loss_g", loss_g, num_step)
                            tb_writer.add_scalar("train/loss_e", loss_e, num_step)
                            tb_writer.add_scalar("train/z_g_grad_norm", z_g_grad_norm, num_step)
                            tb_writer.add_scalar("train/z_e_grad_norm", z_e_grad_norm, num_step)
                ...
                try:
                    def sample_x():
                        if args.prior_type == 'flow':
                            ...
                            with torch.no_grad():
                                x_samples = netG(z_e_k)
                        elif args.prior_type == 'gaussian':
                            z_sample = torch.randn(args.batch_size, args.nz, 1, 1).to(device)
                            with torch.no_grad():
                                x_samples = netG(torch.reshape(z_sample, (z_sample.shape[0], z_sample.shape[1], 1, 1)))
